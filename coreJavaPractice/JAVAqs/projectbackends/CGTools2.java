...
public class CGTools2 {
    public static void main(String[] args) {
        ...
        do {
...

            {
                ...
                // pairCal(p2Cards));
                // System.out.println("~~ rejectedCardSet :: " +
                ...
                if (rejectedCardSet.length == 0) {
                    ...
                    // System.out.println("p1 picked random card " + randomCardp1 + " from
                    // cardset");

                    cardSet = remove(cardSet, randomCardp1);

                    // System.out.println("random card " + randomCardp1 + " picked by p1 is removed
                    // from cardset ");
                    // System.out.println(
                    ...
                    if (accept == 1) {
                        ...
                        // System.out.println("player p1 wants to replace card " + replaceCardp1 + "
                        // from p1cards "
                        // + Arrays.toString(p1Cards));
                        p1Cards = remove(p1Cards, replaceCardp1);
                        p1Cards = add(p1Cards, randomCardp1);
                        // System.out.println("p1 cards unsorted after replacement :: " +
                        // Arrays.toString(p1Cards));
                        p1Cards = sortCards(p1Cards);
                        if (pairCal(p1Cards) == 3) {
                            break;
                        }
                        // System.out.println("p1 cards sorted after replacement :: " +
                        // Arrays.toString(p1Cards) + " pairs "
                        // + pairCal(p1Cards));
                        // System.out.println(
                        // "rejected cardsset before adding replacing card :: " +
                        // Arrays.toString(rejectedCardSet));
                        rejectedCardSet = add(rejectedCardSet, replaceCardp1);
                        rejectedCard = rejectedCardSet[rejectedCardSet.length - 1];
                        // System.out.println(
                        // "rejected cardsset after adding replacing card :: " +
                        // Arrays.toString(rejectedCardSet));
                        // System.out.println("p1 cards ::: "+Arrays.toString(p1Cards)+" pair
                        // :::"+pairCal(p1Cards));
                        // System.out.println("rejected cardset ::: "+Arrays.toString(rejectedCardSet));

                        System.out.println("cardSet         :: " + Arrays.toString(cardSet));
                        System.out.println(
                                "p1Cards         :: " + Arrays.toString(p1Cards) + " pair :: " + pairCal(p1Cards));
                        System.out.println("rejectedCardSet :: " + Arrays.toString(rejectedCardSet));
                    ...
                    rejectedCard = rejectedCardSet[rejectedCardSet.length - 1];
                    System.out.println("p1 cards ::: " + Arrays.toString(p1Cards) + " pair ::: " + pairCal(p1Cards)
                            + " rejected card ::: " + rejectedCard);

                    System.out.println(
                            "please enter 1 or 2 in order to select card from rejected cardset or cardset respectively");
                    ...
                    if (rCardOrCardSetp1 == 1) // START HERE
                    {
                        ...
                        // System.out.println("p1 cards :: " + Arrays.toString(p1Cards) + " pairs " +
                        // pairCal(p1Cards));
                        // System.out.println("rejected cardset :: " +
                        // Arrays.toString(rejectedCardSet));
                        ...
                        System.out.println("cardset          :: " + Arrays.toString(cardSet));
                        System.out.println(
                                "p2 card          :: " + Arrays.toString(p2Cards) + " pair :" + pairCal(p2Cards));
                        System.out.println("rejected cardset :: " + Arrays.toString(rejectedCardSet));
                        ...
                        if (accept == 1) {
                            ...
                            // + pairCal(p1Cards));
                            System.out.println("p1 cards ::: " + Arrays.toString(p1Cards) + " pair ::: "
                                    + pairCal(p1Cards) + " randomCard ::: " + randomCardp1);
                            System.out.println("which card do u want to replace player p1 ? ::");
                            replaceCardp1 = sc.nextInt();
                ...
                int replaceCardp2 = 0;
                rejectedCard = rejectedCardSet[rejectedCardSet.length - 1];
                System.out.println("p2 cards ::: " + Arrays.toString(p2Cards) + " pair :::" + pairCal(p2Cards)
                        + " rejected card ::: " + rejectedCard);
                ...
                if (rCardOrCardSetp2 == 1) // START HERE
                {
                    ...
                    // System.out.println("p2 cards :: " + Arrays.toString(p2Cards) + " pairs " +
                    // pairCal(p2Cards));
                    // System.out.println("rejected cardset :: " +
                    // Arrays.toString(rejectedCardSet));
                    // System.out.println("upper card from rejected cardset :: " + rejectedCard);
                    // System.out.println("p2 cards ::: "+Arrays.toString(p2Cards)+" pair
                    // :::"+pairCal(p2Cards)+" rejected card :::"+rejectedCard+" rejected card set
                    // :::"+Arrays.toString(rejectedCardSet));
                    ...
                    System.out.println("cardset          :: " + Arrays.toString(cardSet));
                    System.out
                            .println("p2 card          :: " + Arrays.toString(p2Cards) + " pair :" + pairCal(p2Cards));
                    System.out.println("rejected cardset :: " + Arrays.toString(rejectedCardSet));
                    ...
                    // System.out.println("p2 picked random card " + randomCardp2 + " from
                    // cardset");

                    cardSet = remove(cardSet, randomCardp2);
                    if (cardSet.length == 0) {
                        System.out.println(
                                "~~~~~~~~~~~~~~~~~~~~~~~RESET DONE ON CARDSET AND REJECTED-CARDSET~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                        for (; rejectedCardSet.length != 0;) {
                            int ele = rejectedCardSet[0];
                            rejectedCardSet = remove(rejectedCardSet, ele);
                            cardSet = add(cardSet, ele);
                    ...
                    // System.out.println("random card " + randomCardp2 + " picked by p2 is removed
                    // from cardset ");
                    // System.out.println(
                    // "cardset after removing random card " + randomCardp2 + " will be :: "
                    // + Arrays.toString(cardSet));
                    int acceptp2 = 0;
                    System.out.println("p1 cards :::" + Arrays.toString(p2Cards) + " pairs::: " + pairCal(p2Cards)
                            + " randomcard::: " + randomCardp2);
                    ...
                    if (acceptp2 == 1) {
                        ...
                        // System.out.println("player p1 wants to replace card " + replaceCardp2 + "
                        // from p1cards "
                        // + Arrays.toString(p2Cards));
                        p2Cards = remove(p2Cards, replaceCardp2);
                        p2Cards = add(p2Cards, randomCardp2);
                        // System.out.println("p2 cards unsorted after replacement :: " +
                        // Arrays.toString(p2Cards));
                        p2Cards = sortCards(p2Cards);
                        if (pairCal(p2Cards) == 3) {
                            break;
                        }
                        // System.out.println("p2 cards sorted after replacement :: " +
                        // Arrays.toString(p2Cards) + " pairs "
                        // + pairCal(p2Cards));
                        // System.out.println(
                        // "rejected cardsset before adding replacing card :: " +
                        // Arrays.toString(rejectedCardSet));
    ...
    static void numToCardName(int num)
    {
        String cardNames[]=new String[52];
         for (int i = 0; i < cardNames.length; i++) {
            if(next4smultiple(i+1)==4){
                cardNames[i]="1";
            }
            else if(next4smultiple(i+1)==8){
                cardNames[i]="2";
            }
            else if(next4smultiple(i+1)==12){
                cardNames[i]="3";
            }
            else if(next4smultiple(i+1)==16){
                cardNames[i]="4";
            }
            else if(next4smultiple(i+1)==20){
                cardNames[i]="5";
            }
            else if(next4smultiple(i+1)==24){
                cardNames[i]="6";
            }
            else if(next4smultiple(i+1)==28){
                cardNames[i]="7";
            }
            else if(next4smultiple(i+1)==32){
                cardNames[i]="8";
            }
            else if(next4smultiple(i+1)==36){
                cardNames[i]="9";
            }
            else if(next4smultiple(i+1)==40){
                cardNames[i]="10";
            }
            else if(next4smultiple(i+1)==44){
                cardNames[i]="J";
            }
            else if(next4smultiple(i+1)==48){
                cardNames[i]="K";
            }
            else if(next4smultiple(i+1)==52){
                cardNames[i]="Q";
            }

         }
         for (int i = 0; i+3 < cardNames.length;i+=4 ) {
            cardNames[i]+="S";
            cardNames[i+1]+="C";
            cardNames[i+2]+="D";
            cardNames[i+3]+="H";
         }
         //System.out.println(Arrays.toString(cardNames));
        //  return cardNames[num];
        System.out.print(cardNames[num]+",");
    }
